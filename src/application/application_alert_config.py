...
class ApplicationAlertMCPTools(BaseInstanaClient):
    ...
    def _validate_string_fields(payload: Dict[str, Any], errors: List[str]) -> None:
        """Validate name and description string fields."""
    ...
    @staticmethod
    def _validate_enum_fields(payload: Dict[str, Any], errors: List[str]) -> None:
        """Validate boundaryScope, evaluationType, granularity, and severity enum fields."""
    ...
    def _validate_required_fields(payload: Dict[str, Any], errors: List[str]) -> None:
        """Validate applications, tagFilterExpression, and timeThreshold required fields."""
    ...
    def _validate_alert_payload(
        ...
        operation: str,
        resource_label: str = "alert_config",
    ) -> Optional[Dict[str, Any]]:
        """
        Validate create/update payload against ApplicationAlertConfig SDK model.
        ...
        errors: List[str] = []
        ApplicationAlertMCPTools._validate_string_fields(payload, errors)
        ApplicationAlertMCPTools._validate_enum_fields(payload, errors)
        ApplicationAlertMCPTools._validate_required_fields(payload, errors)
    ...
    @staticmethod
    def _preflight_alert_config(
        operation: str,
        application_id: Optional[str],
        id: Optional[str],
        created: Optional[int],
    ) -> Optional[Dict[str, Any]]:
        """Validate required parameters before executing an alert_config operation."""
        errors: List[str] = []
        ...
        if operation in ("find", "find_versions", "update", "delete", "enable", "disable",
                         "restore", "update_baseline") and not id:
            errors.append(
        ...
        return {
            "elicitation_needed": True,
            "reason": f"alert_config '{operation}' has {len(errors)} missing required parameter(s)",
            "api_error": errors,
            "message": (
    ...
    async def _dispatch_alert_config(
        self,
        operation: str,
        application_id: Optional[str],
        id: Optional[str],
    ...
    async def execute_alert_config_operation(
        ...
        """
        try:
            preflight = self._preflight_alert_config(operation, application_id, id, created)
            if preflight:
                return preflight

            if operation in ("create", "update"):
                elicitation = self._validate_alert_payload(
                    self._parse_payload(payload), operation, "alert_config"
                )
                if elicitation:
                    return elicitation
